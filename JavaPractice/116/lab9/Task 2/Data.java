...
public class Data
{
  ...
  public Vehicle [] selectionSort(Vehicle [] vh)
  {
		...
		for (int j=0; j<=veh.length-1; j++ )
		{
			...
			{
				if (veh[k].getValue() > veh[index].getValue())
				{
					index=k;					
				}
			}
	  ...
	  public Vector sortVector(Vector vc)
   {
	...
	public String binarySearch(Vehicle [] sort, double key, int start, int end){
		String bs="Object Not Found";
		if ( end >= start ){
			int middle = ( start + end ) / 2;  
			if ( sort[middle].getValue() == key )
				bs= sort[middle].getOwnersName();   // key found 
			else if ( sort[middle].getValue() > key )
				bs=binarySearch(sort, key, start, middle-1); // search left  
			else
				bs=binarySearch(sort, key, middle+1, end); // search right 
		}
		return bs;  // key not found
	}

	public String vectorBinarySearch(Vector sortvect, double key, int start, int end){
		String bsv="Object Not Found";
		int middle=0;
		if(end>=start){
			...
			if(veha.getValue()==key)
				bsv=veha.getOwnersName();
			else if(veha.getValue()>key )
				bsv=vectorBinarySearch(sortvect, key, start, middle-1); // search left
			else
				bsv=vectorBinarySearch(sortvect, key, middle+1, end); // search right
		}
