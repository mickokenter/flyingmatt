...
int main() {
	printf("CS 350 Lab 2 for %s\n\n", "XIAQIN QIU CS350 section 02");  // <-- Fill in
	...
	while (value >= 1 && base >= 2) {
		convert_to_base(value, base, digits, ARRAYLEN);// STUB: call convert_to_base(value, ....)
		print_digits(digits, ARRAYLEN);// STUB: call print_digits(....)
		printf("Enter an integer and base (int < 1 or base < 2 to quit): ");
		scanf("%d %d", &value, &base);
	}
}
...
void convert_to_base(int value, int base, int digits[], int len) {
	...
	if(remain == 0){
		...
		leftover = remain * pow (base, len - position -1);
		printf("%d is too large for %d digits in base %d; %d is leftover\n", value, len, base, leftover);
	}
