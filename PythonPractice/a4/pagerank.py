...
def compute_pagerank(urls, inlinks, outlinks, b=.85, iters=20):
    ...
    """
    ###TODO
    pr = {u: 1. for u in urls}
    for i in list(range(iters)):
        for u in urls:
            sum_ = 0
            for inlink in list(inlinks[u]):
                sum_ = sum_ + pr[inlink] / len(outlinks[inlink])
            pr[u] = (1-b)/len(urls) + b*sum_
    return pr
    pass
...
def get_top_pageranks(inlinks, outlinks, b, n=50, iters=20):
    ...
    ###TODO
    u = list(inlinks.keys())
    pr = sorted(list(compute_pagerank(u, inlinks, outlinks, b=b, iters=iters).items()), key=lambda x: x[1], reverse = True)[:n]
    return pr
    pass
...
def read_links(path):
    ...
    for n in [os.path.basename(nm) for nm in glob.glob(path + os.sep + '*')]:
        lks = get_links(names, open('data\\' + n, mode = 'r', encoding = 'utf-8').read())
        if not n in outlinks:
            outlinks[n] = SortedSet()
        for lk in lks:
            if n != lk:
                outlinks[n].add(lk)
            if not lk in inlinks:
                inlinks[lk] = SortedSet()
            if n != lk:
                inlinks[lk].add(n)
    for n in outlinks:
        if n not in inlinks:
