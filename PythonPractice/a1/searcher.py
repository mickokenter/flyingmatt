...
class Index(object):
    ...
    def compute_doc_lengths(self, index):
        ...
        """
        ###TODO
        dict_set = defaultdict(lambda: 0)        
        for list_index in list(index.values()):
            for doc, doc_freqs in list_index:
                    dict_set[doc] += doc_freqs * doc_freqs
        for key in dict_set:
            dict_set[key] = math.sqrt(dict_set[key])
        return dict_set
        pass
    ...
    def create_champion_index(self, index, threshold=10):
        ...
        dic = dict()
        for term in index:
            champ_sorted_list = sorted(index[term], key = lambda x: x[1], reverse = True)[0:threshold]
            dic[term] = champ_sorted_list
        return dic
        pass
    ...
    def create_tfidf_index(self, docs, doc_freqs):
        ...
        for key in doc_freqs:
            factor = math.log10(len(docs) / doc_freqs[key])
            tfidf = list()
            i = 0
            for doc in docs:
                if key in doc:
                    tfidf.append([i, factor * (1 + math.log10(doc.count(key)))])
                i += 1
            if tfidf != []:
                dic[key] = tfidf
        return dic
    ...
    def search_by_cosine(self, query_vector, index, doc_lengths):
        ...
        for term, weight in query_vector.items():
            for doc_id, doc_weight in index[term]:
                search[doc_id] += weight * doc_weight
        for doc_id in search:
            search[doc_id] /= doc_lengths[doc_id]
        return sorted(search.items(), key=lambda I: I[1], reverse=True)
        pass
    ...
    def search(self, query, use_champions=False):
        ...
        """
        ###TODO
        toked_query = self.tokenize(query)
        vector = self.query_to_vector(toked_query)
        if use_champions:
            return self.search_by_cosine(vector_query, self.champion_index, self.doc_lengths)
        else:
            return self.search_by_cosine(vector_query, self.index, self.doc_lengths)
        pass
