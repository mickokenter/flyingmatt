...
class Index(object):
    ...
    def compute_doc_norms(self, index, n_docs, doc_freqs):
        ...
        """
        ###TODO
        cdn = defaultdict(lambda: 0.)
        for dic in index:
            for doc_id, term_frequency in index[dic]:
                cdn[doc_id] = cdn[doc_id] + ((1 + math.log10(term_frequency)) * math.log10(n_docs / doc_freqs[dic])) * ((1 + math.log10(term_frequency)) * math.log10(n_docs / doc_freqs[dic]))
        for doc_id in cdn:
            cdn[doc_id] = math.sqrt(cdn[doc_id])
        return cdn
        pass
    ...
    def compute_doc_lengths(self, index):
        ...
        """
        ###TODO
        cdl = defaultdict(lambda: 0.)
        for dic in index:
            for doc_id, term_frequency in index[dic]:
                cdl[doc_id] = cdl[doc_id] + term_frequency
        mean = sum(list(cdl.values())) / len(list(cdl.values()))
        return [cdl, mean]
        pass
    ...
    def create_tf_index(self, docs, doc_freqs):
        ...
        """
        ###TODO
        cti = defaultdict(lambda: [])
        cti_index = 1
        for doc in docs:
            for term in set(doc):
                cti[term].append([cti_index, float(doc.count(term))])
            cti_index += 1
        return cti
        pass
        ...
        for term in terms:
            ...
            for doc in self.documents:
                if term.lower() in doc:
                    df += 1
            if df != 0:
                qtv[term] = math.log10(len(self.documents) / df)
        return qtv
        pass
