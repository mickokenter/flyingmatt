...
def parse_relevance_strings(strings):
    ...
    for string in strings:
        prsd_str = re.findall("\d+", string)
        if prsd_str:
            prs[int(parsed_str[0])] = [int(s) for s in parsed_str[1:]]
    return prs
    pass
...
def parse_query_strings(strings):
    ...
    """
    ###TODO
    pqs = defaultdict(lambda: '')
    strings = [s for s in strings if s != '' and s != '\n']
    idx = 0
    for string in strings:
        if re.match('[*]FIND', string):
            idx = int(re.findall('\d+', string)[0])
        elif not re.match('[*]STOP', string):
            pqs[idx] = pqs[idx] + (string.replace('\n', '') + ' ')
    return pqs
    pass
...
def parse_document_strings(strings):
    ...
    doc_temp = ''
    for string in strings:
        if not re.match('[*](STOP|TEXT)', string):
            doc_temp = doc_temp + (string + ' ')
        elif re.match('[*]TEXT', string):
            if not doc_temp == '':
                pds.append(doc_temp)
                doc_temp = ''
        else:
...
def search(query, scorer, index):
    ...
    sorted_list = sorted(scores.keys(), key = lambda key: round(scores[key], 6), reverse = True)
    return sorted_list
    pass
